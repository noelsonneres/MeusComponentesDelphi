...
uses System.SysUtils, System.DateUtils,System.StrUtils,System.IniFiles, System.TypInfo, System.Types, System.UITypes, System.Classes, System.Variants,
  FMX.Types, FMX.Controls, FMX.Forms, FMX.Graphics, FMX.Dialogs, FMX.Menus, FMX.Layouts, FMX.Controls.Presentation,
  FMX.StdCtrls, FMX.Objects, FMX.TabControl, FMX.Edit, FMX.EditBox, FMX.SpinBox, FMX.ListBox, FMX.ScrollBox, FMX.Memo,
  System.ImageList, FMX.ImgList, ACBrBase, ACBrECF, ACBrDevice, FMX.DateTimeCtrls, ACBrRFD, ACBrECFVirtual,
  ACBrECFVirtualBuffer, ACBrECFVirtualPrinter, ACBrECFVirtualNaoFiscal, FMX.ComboEdit, FMX.WebBrowser;
...
type
  TForm1 = class(TForm)
    ...
    cbxModelo: TComboBox;
    seTimeOut: TSpinBox;
    seIntervaloAposComando: TSpinBox;
    speLinBuf: TSpinBox;
    cbxUF: TComboBox;
    chTentar: TCheckBox;
    ...
    pgcMenuFiscalTipo: TTabControl;
    tbsMenuFiscalTipoData: TTabItem;
    tbsMenuFiscalTipoCOO: TTabItem;
    ...
    grptbsMenuFiscalTipoData: TLayout;
    Label25: TLabel;
    edtDtInicial: TDateEdit;
    Label26: TLabel;
    edtDtFinal: TDateEdit;
    grptbsMenuFiscalTipoCOO: TLayout;
    Label27: TLabel;
    edtCOOInicial: TSpinBox;
    Label28: TLabel;
    edtCOOFinal: TSpinBox;
    Label30: TLabel;
    speBarrasLargura: TSpinBox;
    Label31: TLabel;
    speBarrasAltura: TSpinBox;
    Label32: TLabel;
...
uses ACBrECFClass, ACBrConsts, ACBrUtil, ACBrECFBematech, Math, ActiveX, MSHTML, ConfiguraSerial, ACBrPAFClass
, RelatorioGerencialFormatado, VendeItem, EfetuaPagamento, Relatorio, Sobre, uDAV, uDAVOS, uVendaFrenetica;//Relatorio, Sobre, VendeItem, EfetuaPagamento, RelatorioGerencialFormatado, uDAV, uDAVOS, uVendaFrenetica;
...
begin
  ...
  cbMemoHTML.IsChecked := ( ACBrECF1.MemoParams.Values['HTML'] = '1' ) ;

  if (not chRFD.IsChecked) and DirectoryExists( ACBrRFD1.DirRFD ) then
     chRFD.IsChecked := true ;
end;
...
begin
  ...
  StatusPanel0.Text := 'Exception' ;
  AtualizaMemos( False ) ;
  StatusPanel2.Text := E.Message ;
...
begin
  try
     LForm1.Enabled := False ;
     ACBrECF1.Porta := cbxPorta.Text ;

     if cbxModelo.ItemIndex = 0 then
        if not ACBrECF1.AcharECF(true,False) then
        begin
           MessageDlg('Nenhum ECF encontrado.', TMsgDlgType.mtInformation,[TMsgDlgBtn.mbOk],0) ;
           exit ;
        end ;
     ...
     if TabControl1.ActiveTab = tbiECF then
        TabControl1.ActiveTab := tbiCmdResp;
  finally
     LForm1.Enabled := True ;
     cbxModelo.ItemIndex := Integer(ACBrECF1.Modelo) ;
     cbxPorta.Text       := ACBrECF1.Porta ;
...
begin
  ACBrECF1.ReTentar := chTentar.IsChecked ;
end;

procedure TForm1.chBloqueiaClick(Sender: TObject);
begin
  ACBrECF1.BloqueiaMouseTeclado := chBloqueia.IsChecked ;
end;

procedure TForm1.chExibeMsgClick(Sender: TObject);
begin
  ACBrECF1.ExibeMensagem := chExibeMsg.IsChecked ;
end;

procedure TForm1.chArredondaPorQtdClick(Sender: TObject);
begin
  ACBrECF1.ArredondaPorQtd := chArredondaPorQtd.IsChecked ;
end;

procedure TForm1.chDescricaoGrandeClick(Sender: TObject);
begin
  ACBrECF1.DescricaoGrande := chDescricaoGrande.IsChecked ;
end;
...
begin
  StatusPanel2.Text := StringReplace(Mensagem,#10,' ',[rfReplaceAll]) ;
end;
...
begin
  ...
  begin
     if MessageDlg('A Redução Z pode Bloquear o seu ECF até a 12:00pm'+#10+#10+
                  'Continua assim mesmo ?', TMsgDlgType.mtWarning,mbYesNoCancel,0) <> mrYes then
        exit ;

     if MessageDlg('Você tem certeza ?',TMsgDlgType.mtWarning,mbYesNoCancel,0) <> mrYes then
        exit ;
  end ;

  Resp := MessageDlg('Envia hora atual ?',TMsgDlgType.mtConfirmation,mbYesNoCancel,0) ;
  ...
  if not (ACBrECF1.Modelo in [ecfDataRegis, ecfFiscNET]) then
     if ACBrECF1.Estado <> estPagamento then
        MessageDlg('Impressora nao está em Estado de Pagamento'+#10+
                   'Primeiro use SubTotaliza Cupom' ,TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOk],0) ;
...
begin
  ...
  begin
     ...
     ACBrECF1.InfoRodapeCupom.CupomMania := cbxUF.Selected.Text = 'RJ';
     ACBrECF1.InfoRodapeCupom.MinasLegal := cbxUF.Selected.Text = 'MG';
     ACBrECF1.InfoRodapeCupom.ParaibaLegal := cbxUF.Selected.Text = 'PB';
     ACBrECF1.InfoRodapeCupom.NotaLegalDF.Imprimir := cbxUF.Selected.Text = 'DF';
...
Var Resp : TModalResult ;
begin
  Resp := MessageDlg('Arredondar ?', TMsgDlgType.mtConfirmation,mbYesNoCancel,0) ;
...
begin
  ...
  Resp := MessageDlg('Monitorar estado do ECF ?', TMsgDlgType.mtConfirmation,
                             mbYesNoCancel,0) ;
  if Resp = mrCancel then
     exit ;
...
begin
  ...
  try
     ...
     dDatIni := StrToDateTime( StringReplace(cDatIni,'/', FormatSettings.DateSeparator,
                                [rfReplaceAll] ) ) ;
  except
     exit ;
  end ;
  ...
  if not InputQuery('Captura da Memoria Fiscal',
     ...
     exit ;
  try
     dDatFim := StrToDateTime( StringReplace(cDatFim,'/', FormatSettings.DateSeparator,
                                [rfReplaceAll] ) ) ;
  except
     exit
  end ;
...
begin
  ...
  cAliq := StringReplace(StringReplace(cAliq,'.',FormatSettings.DecimalSeparator,[]),
  ...
  if MessageDlg('A aliquota: ['+FloatToStr(nAliq)+'] do Tipo: ['+Tipo+
                ...
                'Confirma a operação ?',TMsgDlgType.mtConfirmation,mbYesNoCancel,0) <> mrYes then
     exit ;
...
begin
  ...
  sValor := StringReplace(StringReplace(sValor,'.',FormatSettings.DecimalSeparator,[]),
                                               ',',FormatSettings.DecimalSeparator,[]) ;
...
begin
  ACBrECF1.GavetaSinalInvertido := chGavetaSinalInvertido.IsChecked ;
end;

procedure TForm1.LerTodasasVariveis1Click(Sender: TObject);
begin
  DataHora1.OnClick(Self);
  NumECF1.OnClick(Self);
  NumLoja1.OnClick(Self);
  NSrie1.OnClick(Self);
  NVerso1.OnClick(Self);
  NumCRO1.OnClick(Self);
  NUltimoCupom1.OnClick(Self);
  SubTotal1.OnClick(Self);
  TotalPago1.OnClick(Self);

  PoucoPapel1.OnClick(Self);
  HorarioVerao2.OnClick(Self);
  Arredonda1.OnClick(Self);

  AliquotasICMS1.OnClick(Self);
  FormasdePagamento1.OnClick(Self);
  CarregaComprovantesNAOFiscais1.OnClick(Self);
  CarregaUnidadesdeMedida1.OnClick(Self);
end;
...
begin
  OpenURL( ExtractFilePath( ExeName ) + edLog.Text);
end;

procedure TForm1.cbMemoHTMLClick(Sender: TObject);
begin
  if cbMemoHTML.IsChecked then
   begin
  ...
  mBobina.Visible  := not cbMemoHTML.IsChecked ;
...
begin
  ...
  StatusPanel1.Text := Format('Anterior: %s - Atual: %s', [sEstAnterior, sEstAtual]);
end;
...
begin
  ...
  try
     try
        ...
        if chRFD.IsChecked then
           ACBrECF1.RFD := ACBrRFD1
        else
           ACBrECF1.RFD := nil ;
     except
        chRFD.OnClick := nil ;
        chRFD.IsChecked := Assigned( ACBrECF1.RFD )  ;
...
procedure TForm1.GravarINI;
  Var ArqINI : String ;
      INI : TIniFile ;
begin
  ArqINI := ChangeFileExt( ExeName,'.ini' ) ;

  INI := TIniFile.Create(ArqINI);
  try
     INI.WriteInteger('ECF','Modelo',cbxModelo.ItemIndex);
     INI.WriteString('ECF','Porta',cbxPorta.Text);
     INI.WriteInteger('ECF','TimeOut',seTimeOut.Text.ToInteger);
     INI.WriteInteger('ECF','IntervaloAposComando',seIntervaloAposComando.Text.ToInteger);
     INI.WriteBool('ECF','TentarNovamente',chTentar.IsChecked);
     INI.WriteBool('ECF','BloqueiaMouseTeclado',chBloqueia.IsChecked);
     INI.WriteBool('ECF','ExibeMsgAguarde',chExibeMsg.IsChecked);
     INI.WriteBool('ECF','ArredondaPorQtd',chArredondaPorQtd.IsChecked);
     INI.WriteBool('ECF','GavetaSinalInvertido',chGavetaSinalInvertido.IsChecked);
     INI.WriteBool('ECF','DescricaoGrande',chDescricaoGrande.IsChecked);
     ...
     INI.WriteBool('RFD','GerarRFD',chRFD.IsChecked);
...
begin
  ...
  try
     ...
     seTimeOut.Value := INI.ReadInteger('ECF','TimeOut',seTimeOut.Text.ToInteger);
     seIntervaloAposComando.Value := INI.ReadInteger('ECF','IntervaloAposComando',seIntervaloAposComando.Text.ToInteger);
     chTentar.IsChecked := INI.ReadBool('ECF','TentarNovamente',chTentar.IsChecked);
     chBloqueia.IsChecked := INI.ReadBool('ECF','BloqueiaMouseTeclado',chBloqueia.IsChecked);
     chExibeMsg.IsChecked := INI.ReadBool('ECF','ExibeMsgAguarde',chExibeMsg.IsChecked);
     chArredondaPorQtd.IsChecked := INI.ReadBool('ECF','ArredondaPorQtd',chArredondaPorQtd.IsChecked);
     chDescricaoGrande.IsChecked := INI.ReadBool('ECF','DescricaoGrande',chDescricaoGrande.IsChecked);
     chGavetaSinalInvertido.IsChecked := INI.ReadBool('ECF','GavetaSinalInvertido',chGavetaSinalInvertido.IsChecked);
     ...
     chRFD.IsChecked := INI.ReadBool('RFD','GerarRFD',chRFD.IsChecked);
...
begin
  ACBrECF1.TimeOut := seTimeOut.Text.ToInteger;
end;

procedure TForm1.seIntervaloAposComandoChange(Sender: TObject);
begin
  ACBrECF1.IntervaloAposComando := seIntervaloAposComando.Text.ToInteger;
end;
...
begin
  ...
  try
     frmSobre.lVersao.Text := 'Ver: '+ECFTeste_VERSAO+' ACBr '+ ACBR_VERSAO ;
     frmSobre.ShowModal ;
  finally
     FreeAndNil(frmSobre) ;
  end ;
...
procedure TForm1.CortaPapel1Click(Sender: TObject);
 Var Resp : TModalResult ;
begin
  Resp := MessageDlg('Corte Parcial ?', TMsgDlgType.mtConfirmation,mbYesNoCancel,0) ;
...
function TForm1.ExeName: string;
begin
  Result := ParamStr(0)
end;
...
begin
  ...
  if not InputQuery('Captura da MFD',
     ...
     exit ;
  try
     dData  := StrToDateTime( StringReplace(cData,'/', FormatSettings.DateSeparator,
                                [rfReplaceAll] ) ) ;
  except
     exit ;
  end ;
...
begin
  ACBrECF1.MaxLinhasBuffer := speLinBuf.Text.ToInteger ;
end;
...
var
  PathArquivo: string;
begin
  if chkMenuFiscalGerarArquivo.IsChecked then
  begin
    ...
    begin
      ...
      if chkMenuFiscalCotepe1704.IsChecked then
      begin
        if pgcMenuFiscalTipo.ActiveTab = tbsMenuFiscalTipoData then
...
begin
  ACBrECF1.ArredondaItemMFD := chArredondamentoItemMFD.IsChecked ;
end;

procedure TForm1.chBarrasImprimeTextoClick(Sender: TObject);
begin
  ACBrECF1.ConfigBarras.MostrarCodigo := chBarrasImprimeTexto.IsChecked;
end;

procedure TForm1.chIgnorarTagsFormatacaoClick(Sender: TObject);
begin
  ACBrECF1.IgnorarTagsFormatacao := chIgnorarTagsFormatacao.IsChecked;
end;
...
begin
  ...
  ACBrECF1.ConfigBarras.LarguraLinha  := speBarrasLargura.Text.ToInteger;
  ACBrECF1.ConfigBarras.Altura        := speBarrasAltura.Text.ToInteger;
  ACBrECF1.ConfigBarras.MostrarCodigo := chBarrasImprimeTexto.IsChecked;
  ACBrECF1.IgnorarTagsFormatacao      := chIgnorarTagsFormatacao.IsChecked;
...
begin
  ACBrECF1.InfoRodapeCupom.CupomMania := chbCupomMania.IsChecked;
end;
