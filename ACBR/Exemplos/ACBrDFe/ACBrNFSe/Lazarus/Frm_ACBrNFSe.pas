...
uses
  ...
  ACBrNFSe, ACBrNFSeDANFSeClass, ACBrNFSeDANFSeRLClass;
...
begin
  ...
  begin
    ...
    Codigo := '';
    if not(InputQuery('Cancelar NFSe', 'Código de Cancelamento (1, 2, 3)', Codigo)) then
      exit;
...
begin
  DataInicial := '';
  if not(InputQuery('Consultar NFSe por Período', 'Data Inicial (DD/MM/AAAA):', DataInicial)) then
    exit;

  DataFinal := '';
  if not(InputQuery('Consultar NFSe por Período', 'Data Final (DD/MM/AAAA):', DataFinal)) then
    exit;

  NumeroNFSe := '';
  if not(InputQuery('Consultar NFSe por Período', 'Numero da NFSe:', NumeroNFSe)) then
    exit;
